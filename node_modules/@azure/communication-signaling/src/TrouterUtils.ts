...
import {
  ...
  StreamingChatMessageStartPayload,
  StreamingChatMessageChunkReceivedPayload,
...
function toChatMessageReceivedEvent<T extends ChatMessageReceivedEvent>(
  ...
  return {
      ...
      streamingMetadata: {
        streamingMessageType: (parsedStreamingPayload as Record<string, StreamingMessageType>)?.contentType,
        streamEndReason: (parsedStreamingPayload as Record<string, StreamEndReason>)?.streamEndReason,
        streamingSequenceNumber: Number((parsedStreamingPayload as Record<string, string>)?.sequenceNumber),
      }
    } as T;
};
...
const toEventPayload = (
  ...
  if (event == "streamingChatMessageStarted") {
    const payload = genericPayload as StreamingChatMessageStartPayload;
    return toChatMessageReceivedEvent(payload, resourceEndpoint, gatewayApiVersion);
  }
...
const toPolicyViolation = (payload: PolicyViolationPayload): PolicyViolation => {
